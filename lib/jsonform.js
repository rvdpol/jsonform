...
formNode.prototype.getFormValues = function (updateArrayPath) {
  ...
  var multiselectIndexes = [];
  // multiselect values have no key for their value
  // so their values are stored properly
  $.each(formArray, function (key, value) {
    if (value.name.indexOf('[]') === -1) {
        return true;
    }
    var fieldName = value.name.substring(0, value.name.indexOf('[]'));
    if (!multiselectIndexes.hasOwnProperty(fieldName)) {
        multiselectIndexes[fieldName] = 0;
    }
    value.name = fieldName + '[' + multiselectIndexes[fieldName] + ']';
    multiselectIndexes[fieldName]++;
  });
